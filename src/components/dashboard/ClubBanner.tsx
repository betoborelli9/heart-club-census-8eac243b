 ...
 * [STATUS]: PRODUÇÃO - VERSÃO 53.0 (RESILIENTE + JSON CLEANER)
 * [CONTEXTO]: Correção de erro de gravação de Mascote e Prioridade Nacional.
 */
...
/** * MÓDULO: UTILITÁRIOS
 * Limpa marcações de markdown do Gemini para evitar erro no JSON.parse
 */
...
serve(async (req) => {
  ...
  try {
    ...
    /** * MÓDULO: BUSCA TÉCNICA (API FOOTBALL)
     * Prioriza Séries Nacionais (A, B, C, D)
     */
    ...
    /** * MÓDULO: BUSCA CRIATIVA (GEMINI 1.5 FLASH)
     * Extração forçada de JSON puro com baixa temperatura.
     */
    let aiData = {
      cor_primaria: "#1a1a1a",
    ...
    if (geminiKey) {
      ...
      JSON: {"cor_primaria": "#HEX", "cor_secundaria": "#HEX", "cor_terciaria": "#HEX", "mascote": "NOME", "tem_feminino": boolean}.`;
      ...
      if (rawText) {
        const cleanJson = cleanGeminiResponse(rawText);
        const parsed = JSON.parse(cleanJson);
        aiData = { ...aiData, ...parsed };
      }
    }

    /** * MÓDULO: GRAVAÇÃO (SUPABASE UPSERT)
     */
    const payload = {
      nome: club_name,
      nome_curto: teamInfo?.team?.code || club_name.substring(0, 3).toUpperCase(),
      api_id: teamInfo?.team?.id?.toString() || null,
