 ...
 * [STATUS]: VERSÃO 22.0 (ENCAPSULAMENTO TOTAL — UNIFIED BORDER)
 * [DESCRIÇÃO]: Banner e Navbar unidos em um único bloco visual.
 * - Borda: Contorno único em volta de todo o conjunto (Banner + Nav).
 * - Divisória: Removida a borda entre o banner e a navbar.
 * - Diagonais: Mantido refinamento estreito (5%).
...
const calculateLuminance = (hex: string): number => {
  if (!hex) return 1;
  const c = hexlength !== 6) return 1;
  const r = parseInt(c.slice(0, 2), 16) / 255;
  const g = parseInt(c.slice(2, 4), 16) / 255;
  const b = parseInt(c.slice(4, 6), 16) / 255;
  return 0.2126 * r.replace("#", "");
  if (c. + 0.7152 * g + 0.0722 * b;
};
...
const ClubBanner = ({
  ...
  const NavItem = ({ icon: Icon, label, path, active, variant }: any) => (
    <button
      ...
      className={`flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all duration-300 text-[10px] font-black uppercase italic ${
        variant === 'danger' ? 'bg-red-600 text-white animate-pulse' :
        variant === 'orange' ? 'text-[#ff6200] border border-[#ff6200]/20 hover:bg-[#ff6200]/10' :
        active ? 'bg-[#ff6200] text-white shadow-[0_0_15px_rgba(255,98,0,0.3)]' :
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4">
      ...
      <div className="overflow-hidden rounded-[2.5rem] border border-white/10 shadow-2xl flex flex-col">
 ...
 * VERSÃO: 22.0
 * CORREÇÕES:
 * - Unificação Visual: Banner e Navbar agora compartilham um container pai com borda única.
 * - Remoção de Bordas Internas: Não há divisória entre o banner e a navegação (border-t-0 removido).
 * - Borda Externa: Contorno fino (border-white/10) aplicado em todo o conjunto para destaque.
 * - Estética: Fundo da navbar (#1a1a1a) integrado como base do container unificado.
 */