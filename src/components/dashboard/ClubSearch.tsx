...
export const ClubSearch = ({ onSelect }: { onSelect: (club: ClubSearchResult) => void }) => {
  ...
  const handleSelect = (club: ClubSearchResult) => {
    console.log("CLICOU NO CLUBE:", club.name); // Para você ver no console
    onSelect(club);
    setResults([]);
    setQuery("");
  };
  ...
  return (
    <div className="relative w-full max-w-md mx-auto" ref={searchRef}>
      ...
      {results.length > 0 && (
        <div className="absolute top-14 left-0 right-0 bg-zinc-950 border border-white/10 rounded-2xl overflow-hidden z-[1000] shadow-[0_20px_50px_rgba(0,0,0,1)] max-h-[400px] overflow-y-auto animate-in fade-in zoom-in duration-200">
          <div className="p-2 border-b border-white/5 bg-zinc-900/50">
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 px-2">Resultados encontrados</span>
          </div>
          {results.map((club) => (
            <button
            ...
            >
              <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center p-2 shrink-0 group-hover:scale-110 transition-transform shadow-lg">
