  ...
  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO result FROM (
    WITH filtered AS (
      SELECT
        v.clube_nome,
        v.created_at,
        -- canonical key: trim + lowercase + remove accents + collapse spaces + strip generic tokens
        regexp_replace(
          regexp_replace(
            unaccent(lower(btrim(v.clube_nome))),
            '\s+(futebol clube|clube de futebol|esporte clube|esporte e clube|sport club|football club|atletico clube|clube atletico|associacao atletica|fc|ec|sc|cf|ac)\s*', ' ', 'gi'
          ),
          '\s+', ' ', 'g'
        ) AS canonical_key
      FROM public.votos v
    ...
    grouped AS (
      SELECT
        ...
        -- pick display name = most frequent original spelling, tie-break by longest
        (
          SELECT clube_nome
          FROM filtered f2
