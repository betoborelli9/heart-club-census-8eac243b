...
CREATE OR REPLACE FUNCTION public.admin_get_votes_with_tracking()
 RETURNS TABLE(voto_id uuid, clube_nome text, user_email text, user_nome text, ip_address text, cep text, cidade text, estado text, is_suspicious boolean, status_aprovacao text, motivo_suspicao text, created_at timestamp with time zone, fraud_score integer, sympathy_1 text, sympathy_2 text, sympathy_3 text, sympathy_4 text)
 LANGUAGE plpgsql
 SECURITY DEFINER
