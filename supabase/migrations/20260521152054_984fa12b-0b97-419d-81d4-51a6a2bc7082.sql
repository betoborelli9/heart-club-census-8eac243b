...
DECLARE
  ...
  v_all boolean := (p_club_name IS NULL OR length(trim(p_club_name)) = 0);
BEGIN
  IF p_level = 'continent' THEN
    SELECT json_agg(row_to_json(t)) INTO result FROM (
      SELECT COALESCE(NULLIF(v.voto_continente, ''), 'Não informado') as region, count(*) as votes
      FROM public.votos v WHERE (v_all OR v.clube_nome = p_club_name) AND v.is_original_vote = true AND v.status_aprovacao = 'aprovado'
  ...
  ELSIF p_level = 'country' THEN
    SELECT json_agg(row_to_json(t)) INTO result FROM (
      ...
      WHERE (v_all OR v.clube_nome = p_club_name) AND v.is_original_vote = true AND v.status_aprovacao = 'aprovado'
      GROUP BY 1 ORDER BY votes DESC
    ) t;
...
BEGIN
  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO result FROM (
    ...
    FROM public.votos v
    LEFT JOIN public.profiles p ON p.id = v.user_id AND p.address_confirmed = true
    WHERE (v_all OR v.clube_nome = p_club_name)
