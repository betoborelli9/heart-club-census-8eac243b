...
DECLARE
  ...
  v_locations jsonb := jsonb_build_array(
    -- Goiânia repetida várias vezes p/ aumentar peso (foco do teste de bairros)
  ...
  -- Bairros por cidade (foco em Goiânia + amostras das maiores BR p/ choropleth)
  v_bairros_por_cidade jsonb := jsonb_build_object(
    'Goiânia', jsonb_build_array(
      'Setor Bueno','Setor Marista','Setor Oeste','Setor Sul','Setor Aeroporto',
      'Jardim Goiás','Setor Central','Setor Coimbra','Setor Campinas','Setor Universitário',
      'Vila Nova','Setor Pedro Ludovico','Setor Bela Vista','Park Lozandes','Alto da Glória',
      'Vila Brasília','Jardim América','Setor Criméia Oeste','Setor Criméia Leste','Vila Maria José',
      'Setor Negrão de Lima','Setor Vila Redenção','Conjunto Vera Cruz','Setor Faiçalville','Residencial Eldorado',
      'Setor Jaó','Setor Cândida de Morais','Conjunto Itatiaia','Setor Garavelo','Residencial Planalto'
    ),
    'São Paulo', jsonb_build_array(
      'Vila Mariana','Pinheiros','Itaim Bibi','Moema','Jardins',
      'Vila Madalena','Tatuapé','Mooca','Santana','Brooklin'
    ),
    'Rio de Janeiro', jsonb_build_array(
      'Copacabana','Ipanema','Leblon','Botafogo','Tijuca',
      'Barra da Tijuca','Flamengo','Lapa','Centro','Méier'
    ),
    'Belo Horizonte', jsonb_build_array(
      'Savassi','Lourdes','Funcionários','Pampulha','Buritis',
      'Centro','Belvedere','Cidade Nova','Sion','Anchieta'
    ),
    'Brasília', jsonb_build_array(
      'Asa Sul','Asa Norte','Lago Sul','Lago Norte','Sudoeste',
      'Noroeste','Águas Claras','Taguatinga','Ceilândia','Guará'
    ),
    'Curitiba', jsonb_build_array(
      'Batel','Centro','Água Verde','Bigorrilho','Cabral',
      'Juvevê','Mercês','Rebouças','Portão','Boa Vista'
    ),
    'Porto Alegre', jsonb_build_array(
      'Moinhos de Vento','Bela Vista','Petrópolis','Centro Histórico','Cidade Baixa',
      'Menino Deus','Auxiliadora','Higienópolis','Mont Serrat','Tristeza'
    ),
    'Salvador', jsonb_build_array(
      'Barra','Pituba','Itaigara','Caminho das Árvores','Graça',
      'Ondina','Rio Vermelho','Pelourinho','Federação','Stiep'
    )
  );
  v_nomes text[] := ARRAY[
...
BEGIN
  ...
  FOR i IN 1..p_quantidade LOOP
    ...
    -- Sorteia bairro se a cidade tiver lista cadastrada
    v_bairro := NULL;
    v_bairros_arr := v_bairros_por_cidade -> v_cidade;
    IF v_bairros_arr IS NOT NULL AND jsonb_array_length(v_bairros_arr) > 0 THEN
      -- Distribuição não-uniforme (random()^1.6) gera gradiente p/ choropleth
      v_bairro := v_bairros_arr ->> floor(power(random(), 1.6) * jsonb_array_length(v_bairros_arr))::int;
    END IF;
  ...
  RETURN json_build_object(
    ...
    'modo', 'ficticio_diverso_com_bairros'
  );
END;
$function$;