-- Adiciona telefone (WhatsApp) aos RPCs de acesso, pra uso exclusivo do
-- Master Admin em publicidade/prospecção — nunca exposto a parceiros/torcedores
-- (essas RPCs já são SECURITY DEFINER restritas a admin/master).
...
BEGIN
  ...
  SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) INTO result FROM (
    ...
    GROUP BY a.user_id, p.nome_exibicao, u.email, p.telefone
    ORDER BY count(*) DESC
    LIMIT 200
  ) t;
