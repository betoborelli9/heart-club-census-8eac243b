...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      v.cep,
      COALESCE(NULLIF(v.bairro, ''), NULLIF(p.bairro, ''), NULLIF(v.voto_bairro_gps, '')) AS bairro,
      COALESCE(NULLIF(v.cidade, ''), NULLIF(v.voto_cidade_gps, ''), NULLIF(v.voto_cidade, ''), NULLIF(p.cidade, '')) AS cidade,
      COALESCE(NULLIF(v.estado, ''), NULLIF(p.estado, '')) AS estado,
  ...
  refs AS (
    SELECT
      i.indicado_id AS user_id,
      i.codigo_usado,
      pe.nome_exibicao AS ambassador_name
    FROM public.indicacoes i
    LEFT JOIN public.profiles pe ON pe.id = i.embaixador_id
  )
  SELECT
    f.id AS voto_id,
    f.clube_nome::text,
    ...
    f.sympathy_3::text,
    f.sympathy_4::text,
    CASE WHEN r.user_id IS NOT NULL THEN 'embaixador' ELSE 'organico' END::text AS referral_source,
    r.ambassador_name::text AS referral_ambassador_name,
    r.codigo_usado::text AS referral_code
  FROM final f
  LEFT JOIN refs r ON r.user_id = f.user_id
